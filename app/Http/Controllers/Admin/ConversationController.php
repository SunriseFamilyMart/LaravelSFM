...
class ConversationController extends Controller
{
    ...
    // Show all conversations for a sales person
    // Show all sales person messages
    public function index(Request $request)
    {
        // LEFT PANEL: Sales people list
        $salesPeople = Conversation::with('salesPerson')
            ->whereNotNull('sales_person_id')
            ->select('sales_person_id')
            ->groupBy('sales_person_id')
            ->get();

        // RIGHT PANEL: selected person's chats
        $selectedId = $request->sales_person_id;

        $conversations = [];
        if ($selectedId) {
            $conversations = Conversation::with('salesPerson')
                ->where('sales_person_id', $selectedId)
                ->orderBy('created_at', 'asc')
                ->get();
        }

        return view('sales_chat.index', compact('salesPeople', 'conversations', 'selectedId'));
    }
