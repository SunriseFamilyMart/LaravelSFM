...
{}
    ...
    public function deliveryFeeSetup(Request $request): Factory|\Illuminate\Foundation\Application|View|Application
    {
        ...
        $branches = $this->branch
            ...
            }, 'weight_settings_status', 'weight_charge_type', 'weight_unit', 'weight_range'])
            ->get(['id', 'name', 'status']);
        ...
        try {
            ...
            foreach ($fieldArray as $field) {
                if (!array_key_exists($field, $firstRow)) {
                    return response()->json([
                        'status' => 'error',
                        'message' => translate('Invalid file format. Missing field: ') . $field
                    ], 422);
                }
            }
    ...
    public function freeDeliveryOverAmountSetup(Request $request)
    {
        $status = $request->has('free_delivery_status') ? 1 : 0;
