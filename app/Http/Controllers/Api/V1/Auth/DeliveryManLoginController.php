    ...
    public function registration(Request $request): JsonResponse
    {
        ...
        try {
            ...
            if (isset($emailServices['status']) && $emailServices['status'] == 1) {
                Mail::to(Helpers::get_business_settings('email_address'))->send(new \App\Mail\Admin\SubscribeNewsletter($deliveryman->email));
            }
        } catch (\Exception $e) {
        }
    ...
    public function login(Request $request): JsonResponse
    {
        ...
        if (isset($deliveryman) && $deliveryman->application_status != 'approved') {
            return response()->json([
                'errors' => [
                    ['code' => 'auth-001', 'message' => 'Not verified.']
                ]
            ], 401);
        }

        $data = [
            'email' => $deliveryman->email ?? null,   // actual email from database
        ...
        if (isset($deliveryman)) {
            ...
            if (auth('delivery_men')->attempt($data)) {
                ...
                if (
                    isset($deliveryman->temp_block_time) &&
                    Carbon::parse($deliveryman->temp_block_time)->DiffInSeconds() <= $tempBlockTime
                ) {
            ...
            // Failed login handling (hit count, block logic)
            if (
                isset($deliveryman->temp_block_time) &&
                Carbon::parse($deliveryman->temp_block_time)->DiffInSeconds() <= $tempBlockTime
            ) {
