                    ...
                    <div class="card-header flex-wrap align-items-start border-0">
                        <div class="order-invoice-left">
                            ...
                            <span><i class="tio-date-range"></i>
                                ...
                                {{ date(config('time_format'), strtotime($order['created_at'])) }}
                            </span>
    ...
    $current = \App\Model\DeliveryHistory::where(['deliveryman_id' => $order['delivery_man_id'], 'order_id' => $order['id']])->latest()->first(),
                                            ...
                                            @if (isset($origin))
                                                <a class="btn btn-outline-info font-semibold" target="_blank"
                                                    title="{{ translate('Deliveryman Last Location') }}"
                                    ...
                                    @if ($detail->product_details != null)
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-right">
                                                ...
                                                @php($priceAfterDiscount += $amount - $totalItemDiscount)
                                                @php($subTotal += $priceAfterDiscount)
                        ...
                        <div class="row justify-content-md-end mb-3 mt-4">
                            <div class="col-md-9 col-lg-8">
                                <dl class="row text-right justify-content-end">
                                    ...
                                    <dd class="col-6 col-xl-5 pr-5">
                                        {{ Helpers::set_symbol($total + $deliveryCharge + $updatedTotalTax - $order['coupon_discount_amount'] - $order['extra_discount'] + $order['weight_charge_amount']) }}
                                        <hr>
                                    </dd>
                                    @if ($order->payments && $order->payments->where('payment_status', 'complete')->isNotEmpty())
                                        @foreach ($order->payments->where('payment_status', 'complete') as $payment)
                                            ...
                                            <dd class="col-6 col-xl-5 pr-5">
                                                {{ Helpers::set_symbol($payment->amount) }}
                                            </dd>
                                        @endforeach
                                        <?php
                                        $totalPaid = $order->payments->where('payment_status', 'complete')->sum('amount');
                                        $totalOrder = $total + $deliveryCharge + $updatedTotalTax - $order['coupon_discount_amount'] - $order['extra_discount'] + $order['weight_charge_amount'];
                                        $dueAmount = $totalOrder - $totalPaid;
                                        ?>
                                        <dt class="col-6 text-left">
                                            <div class="ml-auto max-w-150px">
                                                <span>
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <div class="dropdown-menu text-capitalize" aria-labelledby="dropdownMenuButton">
                                            @if ($order['payment_method'] == 'offline_payment' && $order->offline_payment?->status != 1)
                                                <a class="dropdown-item offline-payment-verify-alert"
                                                    data-message="{{ translate('You can not change order status to this status. Please Check & Verify the payment information whether it is correct or not. You can only change order status to failed or cancel if payment is not verified.') }}"
                                ...
                                <div class="hs-unfold w-100 mt-3">
                                    ...
                                    <div class="dropdown">
                                        ...
                                        @else
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                <a class="dropdown-item route-alert"
                                                    data-route="{{ route('branch.orders.payment-status', ['id' => $order['id'], 'payment_status' => 'paid']) }}"
                                                    data-message="{{ translate('Change status to paid ?') }}"
                                                    href="javascript:">{{ translate('paid') }}</a>
                                                <a class="dropdown-item route-alert"
                                                    data-route="{{ route('branch.orders.payment-status', ['id' => $order['id'], 'payment_status' => 'unpaid']) }}"
                                                    data-message="{{ translate('Change status to unpaid ?') }}"
                                                    href="javascript:">{{ translate('unpaid') }}</a>
                                            </div>
                                        @endif
                                    </div>
                                ...
                                @foreach ($deliverymanList as $deliveryman)
                                    <li class="list-group-item">
                                        ...
                                        <a class="btn btn-primary btn-xs float-right assign-deliveryman"
                                            data-id="{{ $deliveryman['id'] }}">{{ translate('assign') }}</a>
                                    </li>
                                @endforeach
                            </ul>
                        </div>
    ...
    <div class="modal fade" tabindex="-1" role="dialog" aria-labelledby="editArea" id="editArea"
        ...
        <div class="modal-dialog modal-md" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('branch.orders.update-order-delivery-area', ['order_id' => $order->id]) }}"
                    method="post">
                    @csrf
                    <div class="modal-body">
                        <div class="row">
...
@push('script_2')
    <script>
        ...
        $('.offline-payment-verify-alert').on('click', function() {
        ...
        $('.assign-deliveryman').on('click', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.status == true) {
                        toastr.success(
                            '{{ translate('Deliveryman successfully assigned/changed') }}', {
                                CloseButton: true,
                                ProgressBar: true
                            });
                    } else {
        ...
        $('.verify-offline-payment').on('click', function() {
            let status = $(this).data('status');
            console.log(status);
            $.ajax({
                type: "GET",
                url: '{{ url('/') }}/branch/orders/verify-offline-payment/{{ $order['id'] }}/' +
                    status,
                success: function(data) {
                    location.reload();
                    if (data.status == true) {
