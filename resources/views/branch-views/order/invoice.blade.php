        ...
        <div class="row justify-content-center" id="printableArea">
            ...
            <div class="initial-38-1">
                ...
                <div class="row mt-3">
                    ...
                    <div class="col-12">
                        @if ($order->store_id && $order->store)
                            <h5>
                                <i class="tio-shop mr-1"></i>{{ translate('Store Name') }} :<span class="font-light">{{ $order->store->store_name ?? $order->store->customer_name }}</span>
                            </h5>
                            @if($order->store->phone_number)
                                <h5>
                                    {{ translate('phone') }} :<span class="font-light">{{ $order->store->phone_number }}</span>
                                </h5>
                            @endif
                            @if($order->store->address)
                                <h5 class="text-break">
                                    {{ translate('address') }} :<span class="font-light">{{ $order->store->address }}</span>
                                </h5>
                            @endif
                                    ...
                                    @if(count(json_decode($detail['variation'],true))>0)
                                        ...
                                        @foreach(json_decode($detail['variation'],true)[0] ?? json_decode($detail['variation'],true) as $key1 =>$variation)
                                            <div class="font-size-sm text-body">
                                                ...
                                                <span class="font-weight-bold">{{$variation}} {{$key1=='price'? Helpers::currency_symbol():''}}</span>
                                            </div>
                                        @endforeach
                                    @endif
                ...
                <div class="px-3">
                    ...
                    <span class="initial-38-5">---------------------------------------------------------------------------------</span>
                    <span class="d-block text-center">{{ Helpers::get_business_settings('footer_text')}}</span>
                </div>
            </div>
        </div>
    </div>
