                    ...
                    <div class="card-header flex-wrap align-items-start border-0">
                        <div class="order-invoice-left">
                            <h1 class="page-header-title">
                                ...
                                <span
                                    class="badge badge-soft-info py-2 px-3">{{ $order->branch ? $order->branch->name : translate('Branch deleted!') }}</span>
                            </h1>
...
<p class="mb-1">
    ...
    $current = \App\Model\DeliveryHistory::where(['deliveryman_id' => $order['delivery_man_id'], 'order_id' => $order['id']])->latest()->first()
                                            ...
                                            @endif
                                        @else
                                            <a class="btn btn-outline-info font-semibold last-location-view"
                            ...
                            <div class="text-right mt-3 order-invoice-right-contents text-capitalize">
                                ...
                                @if ($order->trip_number)
                                    <h6>
                                        <span class="text-body mr-2">{{ translate('Trip Number') }} :</span>
                                        <span class="badge badge-soft-primary ml-2">{{ $order->trip_number }}</span>
                                    </h6>
                                @endif
                                ...
                                @if (!in_array($order['payment_method'], ['cash_on_delivery', 'wallet_payment', 'offline_payment']))
                                    <h6 class="text-capitalize">
                                        @if ($order['transaction_reference'] == null && $order['order_type'] != 'pos')
                                            <span class="text-body mr-2"> {{ translate('reference') }}
                                                {{ translate('code') }}
                                                :</span>
                                        ...
                                        @elseif($order['order_type'] != 'pos')
                                            <span class="text-body mr-2">{{ translate('reference') }}
                                                ...
                                                {{ $order['transaction_reference'] }}</span>
                                        @endif
                                    </h6>
                                @endif

                                <h6>
                                    <span class="text-body mr-2">{{ translate('payment') }} {{ translate('status') }} :
                                    </span>
                            ...
                            <table
                                ...
                                @foreach ($order->details as $detail)
                                    ...
                                    @if ($detail->product_details != null)
                                        @php($product = $detail->product_details)
                                        <tr>
                                            <td>
                                                {{ $loop->iteration }}
                                            </td>
                                            ...
                                            <td class="text-right">
                                                <h6>{{ Helpers::set_symbol($detail['discount_on_product'] * $detail['quantity']) }}
                                                </h6>
                                            </td>
                                            <td class="text-right">
                                ...
                                <dl class="row text-right justify-content-end">
                                    ...
                                    <dt class="col-6 text-left">
                                        <div class="ml-auto max-w-150px">
                                            ...
                                            {{ $vatStatus == 'included' ? translate('(included)') : '' }}:
                                        </div>
                                    </dt>
                                    <dd class="col-6 col-xl-5 pr-5">
                    ...
                    <button type="button" class="btn btn-info" data-bs-toggle="modal" data-bs-target="#editLogsModal">
                        View Edit Logs
                    </button>
                    <!-- Edit Logs Modal -->
                    <div class="modal fade" id="editLogsModal" tabindex="-1" aria-labelledby="editLogsModalLabel"
                        aria-hidden="true">
                        <div class="modal-dialog modal-lg">
                            <div class="modal-content">

                                <div class="modal-header">
                                    <h5 class="modal-title" id="editLogsModalLabel">Order Edit Logs</h5>
                                ...
                                <div class="modal-body">
                                    ...
                                    @if ($order->editLogs->count() > 0)
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-bordered table-striped align-middle">
                                                ...
                                                <tbody>
                                                    @foreach ($order->editLogs as $log)
                                                        <tr>
                                                            <td>{{ $log->orderDetail->product->name ?? 'N/A' }}</td>

                                                            <td>
                                                                {{ $log->deliveryMan->f_name ?? '' }}
                                                                {{ $log->deliveryMan->l_name ?? '' }}
                                                            </td>
                                                            ...
                                                            <td class="text-center fw-bold">
                                                                {{ $log->old_quantity }} →
                                                                <span
                                                                    class="{{ $log->new_quantity > $log->old_quantity ? 'text-success' : 'text-danger' }}">
                                                                    {{ $log->new_quantity }}
                                                                </span>
                                                            </td>
                                                            ...
                                                            <td class="text-center fw-bold">
                                                                ...
                                                                <span
                                                                    class="{{ $log->new_price > $log->old_price ? 'text-success' : 'text-danger' }}">
                                    ...
                                    @else
                                        <p class="text-center text-muted">No edit logs found.</p>
                                    @endif
            ...
            <div class="col-lg-4 order-print-area-right">
                            ...
                            <div class="card mt-3">
                                <div class="card-body text-center">
                                    ...
                                    @else
                                        ...
                                        <p class="text-danger">
                                            {{ translate('please verify the payment before confirm order') }}</p>
                                        <div class="mt-3">
                                            <button class="btn btn--primary" type="button" id="verifyPaymentButton"
                                                data-id="{{ $order['id'] }}" data-target="#payment_verify_modal"
                        ...
                        <div class="card-body">
                            @if ($order['order_type'] != 'pos')
                                {{-- <div class="hs-unfold w-100">
                                    <span
                                        class="d-block form-label font-bold mb-2">{{ translate('Change Order Status') }}:</span>
                                    <div class="dropdown">
                                        <button
                                        ...
                                        <div class="dropdown-menu text-capitalize" aria-labelledby="dropdownMenuButton">
                                            @if ($order['payment_method'] == 'offline_payment' && $order->offline_payment?->status != 1)
                                                <a class="dropdown-item manage-status" href="#"
                                                    data-status="pending">{{ translate('pending') }}</a>
                                                <a class="dropdown-item manage-status" href="#"
                                                    data-status="confirmed">{{ translate('confirmed') }}</a>
                                                <a class="dropdown-item manage-status" href="#"
                                                    data-status="packaging">{{ translate('packaging') }}</a>
                                                <a class="dropdown-item manage-status" href="#"
                                                    data-status="out_for_delivery">{{ translate('out_for_delivery') }}</a>
                                                <a class="dropdown-item manage-status" href="#"
                                                    data-status="delivered">{{ translate('delivered') }}</a>
                                                <a class="dropdown-item manage-status" href="#"
                                                    data-status="returned">{{ translate('returned') }}</a>
                                                <a class="dropdown-item manage-status"
                                                    href="{{ route('admin.orders.status', ['id' => $order['id'], 'order_status' => 'failed']) }}"
                                                    data-order_status="failed">{{ translate('failed') }}</a>
                                                <a class="dropdown-item manage-status"
                                                    href="{{ route('admin.orders.status', ['id' => $order['id'], 'order_status' => 'canceled']) }}"
                                                    data-order_status="canceled">{{ translate('canceled') }}</a>
                                            @else
                                                <a class="dropdown-item manage-status"
                                                    href="{{ route('admin.orders.status', ['id' => $order['id'], 'order_status' => 'pending']) }}"
                                                    data-order_status="pending">{{ translate('pending') }}</a>
                                                <a class="dropdown-item manage-status"
                                                    href="{{ route('admin.orders.status', ['id' => $order['id'], 'order_status' => 'confirmed']) }}"
                                                    data-order_status="confirmed">{{ translate('confirmed') }}</a>
                                                <a class="dropdown-item manage-status"
                                                    href="{{ route('admin.orders.status', ['id' => $order['id'], 'order_status' => 'processing']) }}"
                                                    data-order_status="packaging">{{ translate('packaging') }}</a>
                                                <a class="dropdown-item manage-status"
                                                    href="{{ route('admin.orders.status', ['id' => $order['id'], 'order_status' => 'out_for_delivery']) }}"
                                                    data-order_status="out_for_delivery">{{ translate('out_for_delivery') }}</a>
                                                <a class="dropdown-item manage-status"
                                                    href="{{ route('admin.orders.status', ['id' => $order['id'], 'order_status' => 'delivered']) }}"
                                                    data-order_status="delivered">{{ translate('delivered') }}</a>
                                                <a class="dropdown-item manage-status"
                                                    href="{{ route('admin.orders.status', ['id' => $order['id'], 'order_status' => 'returned']) }}"
                                                    data-order_status="returned">{{ translate('returned') }}</a>
                                                <a class="dropdown-item manage-status"
                                                    href="{{ route('admin.orders.status', ['id' => $order['id'], 'order_status' => 'failed']) }}"
                                                    data-order_status="failed">{{ translate('failed') }}</a>
                                                <a class="dropdown-item manage-status"
                                    ...
                                    <div class="dropdown">
                                        ...
                                        @if ($order['payment_method'] == 'offline_payment' && $order->offline_payment?->status != 1)
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                <a class="dropdown-item offline-payment"
                                                    data-message="{{ translate('You can not change status of unverified offline payment') }}"
                                        ...
                                        @else
                                            <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                <a class="dropdown-item change-payment-status" data-status="paid"
                                                    data-route="{{ route('admin.orders.payment-status', ['id' => $order['id'], 'payment_status' => 'paid']) }}">{{ translate('paid') }}</a>
                                                <a class="dropdown-item change-payment-status" data-status="unpaid"
                                                    data-route="{{ route('admin.orders.payment-status', ['id' => $order['id'], 'payment_status' => 'unpaid']) }}">{{ translate('unpaid') }}</a>
                                            </div>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="mt-3">
                                    <span
                                        class="d-block form-label mb-2 font-bold">{{ translate('Delivery Date & Time') }}:</span>
                                    <div class="d-flex flex-wrap g-2">
                                        ...
                                        <div class="hs-unfold w-0 flex-grow min-w-160px">
                                            <select class="custom-select custom-select time_slote" name="timeSlot"
                                                data-id="{{ $order['id'] }}">
                                                <option disabled selected>{{ translate('select') }}
                                                    {{ translate('Time Slot') }}</option>
                                                @foreach (\App\Model\TimeSlot::all() as $timeSlot)
                                                    <option value="{{ $timeSlot['id'] }}"
                                                        {{ $timeSlot->id == $order->time_slot_id ? 'selected' : '' }}>
                                                        {{ date(config('time_format'), strtotime($timeSlot['start_time'])) }}
                                                        -
                                ...
                                @if ($order->delivery_man)
                                    <div class="card mt-2">
                                        <b class="ms-2 text-muted">
                                            {{ translate('If the order is not delivered or delivery man is not available, please change the delivery man') }}
                                        </b>
                                        <div class="card-body">
                                            <h5 class="card-title mb-3 d-flex flex-wrap align-items-center"> <span
                                                    class="card-header-icon"> <i class="tio-user"></i> </span>
                                            ...
                                            <div class="media align-items-center deco-none customer--information-single">
                                                <div class="avatar avatar-circle"> <img class="avatar-img"
                                                        src="{{ $order->delivery_man->imageFullPath }}"
                                                        alt="{{ translate('Image Description') }}"> </div>
                                                <div class="media-body"> <a
                                                    ...
                                                    </a> <span class="text--title font-semibold d-flex align-items-center">
                                                        <i class="tio-shopping-basket-outlined mr-2"></i>
                                                        {{ \App\Model\Order::where(['delivery_man_id' => $order['delivery_man_id'], 'order_status' => 'delivered'])->count() }}
                                                        {{ translate('orders_delivered') }} </span> <span
                                                        class="text--title font-semibold d-flex align-items-center"> <i
                                                            class="tio-call-talking-quiet mr-2"></i> <a
                                                            href="Tel:{{ $order->delivery_man['phone'] }}">{{ $order->delivery_man['phone'] }}</a>
                                                    </span> <span
                                                        class="text--title font-semibold d-flex align-items-center"> <i
                                                            class="tio-email-outlined mr-2"></i> <a
                                                            href="mailto:{{ $order->delivery_man['email'] }}">{{ $order->delivery_man['email'] }}</a>
                                                    </span> </div>
                                            </div>
                                        </div>
                ...
                <div class="card mt-2">
                    <div class="card-body">
                        ...
                        @if ($order->user_id != null && isset($order->customer))
                            ...
                            <div class="media align-items-center deco-none customer--information-single">
                                <div class="avatar avatar-circle">
                                    <img class="avatar-img" src="{{ $order->customer->imageFullPath }}"
                                        alt="{{ translate('Image Description') }}">
                                </div>
                                <div class="media-body">
                                    ...
                                    <span>{{ \App\Model\Order::where('user_id', $order->user_id)->count() }}
                        ...
                        @elseif ($order->store_id != null && isset($order->store))
                            {{-- Store Info --}}
                            <div class="media align-items-center deco-none customer--information-single">
                                <div class="avatar avatar-circle">
                                    <img class="avatar-img" src="{{ asset('storage/' . $order->store->store_photo) }}"
                                        alt="{{ translate('Store Image') }}">
                                ...
                                <div class="media-body">
                                    ...
                                    <span class="text--title font-semibold d-block">
                                        <i class="tio-call-talking-quiet mr-2"></i>
                                        <a
                                            href="Tel:{{ $order->store->phone_number }}">{{ $order->store->phone_number }}</a>
                                    </span>
                                    <span class="text--title">
                        ...
                        <div class="card-body">
                            ...
                            <div class="d-flex flex-wrap gap-2 align-items-center">
                                @foreach ($order->order_image as $orderImage)
                                    <a class="avatar m-1 w-75px h-auto"
                                        href="{{ asset('storage/app/public/order/' . $orderImage->image) }}"
                                        data-lightbox>
                                        <img class="aspect-1 avatar-img object-cover"
                                            src="{{ asset('storage/app/public/order/' . $orderImage->image) }}"
                                            alt="{{ translate('Image Description') }}">
                                    </a>
                                @endforeach
                            </div>
                ...
                <div class="card mt-2">
                    <div class="card-body">
                        ...
                        <div class="media align-items-center deco-none resturant--information-single">
                            <div class="avatar avatar-circle">
                                <img class="avatar-img w-75px" src="{{ $order->branch?->imageFullPath }}"
                                    alt="{{ translate('Image Description') }}">
                            </div>
                            <div class="media-body">
                                @if (isset($order->branch))
                                    ...
                                    <span>{{ \App\Model\Order::where('branch_id', $order['branch_id'])->count() }}
                                        {{ translate('Orders') }}</span>
                                    <span class="text--title font-semibold d-block">
                                        <i class="tio-call-talking-quiet mr-2"></i>
                                ...
                                @else
                                    <span
                                        class="fz--14px text--title font-semibold text-hover-primary d-block">{{ translate('Branch Deleted') }}</span>
                                @endif
                            </div>
                        </div>
                        @if (isset($order->branch))
    ...
    <div class="modal fade bd-example-modal-sm" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel"
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title h4" id="mySmallModalLabel">{{ translate('reference') }}
                        {{ translate('code') }} {{ translate('add') }}</h5>
                                ...
                                @foreach ($deliverymanList as $deliveryman)
                                    <li class="list-group-item">
                                        ...
                                        <a class="btn btn-primary btn-xs float-right assign-deliveryman"
                                            data-deliveryman-id="{{ $deliveryman['id'] }}">{{ translate('assign') }}</a>
                                    </li>
                                @endforeach
                            </ul>
                        </div>
                ...
                <div class="modal-content">
                    ...
                    <div class="card">
                        <div class="modal-body mx-2">
                            <p class="text-danger">
                                {{ translate('Please Check & Verify the payment information whether it is correct or not before confirm the order.') }}
                            </p>
                    ...
                    <div class="btn--container justify-content-end my-2 mx-3">
                        <a type="reset" class="btn btn--reset verify-offline-payment"
                                    ...
                                    <select name="selected_area_id" class="form-control js-select2-custom-x mx-1"
                                        ...
                                        @foreach ($branch->delivery_charge_by_area as $area)
                                            <option value="{{ $area['id'] }}"
                                                {{ isset($order->order_area) && $order->order_area->area_id == $area['id'] ? 'selected' : '' }}
...
@push('script_2')
    ...
    <script>
        ...
        $('.manage-status').on('click', function(event) {
            ...
            if (status === 'pending' || status === 'confirmed' || status === 'packaging' || status ===
                'out_for_delivery' || status === 'delivered' || status === 'returned') {
                var message =
                    '{{ translate('You can not change order status to this status. Please Check & Verify the payment information whether it is correct or not. You can only change order status to failed or cancel if payment is not verified.') }}';
            ...
            Swal.fire({
                ...
                if (result.value) {
                    ...
                    $.post({
                        ...
                        success: function(data) {
                            toastr.success(
                                '{{ translate('Time Slot Change successfully') }}');
                            location.reload();
                        }
                    });
                }
