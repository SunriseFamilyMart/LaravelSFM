        ...
        <div class="row justify-content-center" id="printableArea">
            <div class="col-md-12">
                <div class="text-center">
                    <input type="button" class="btn btn--primary non-printable text-white" data-type="printableArea"
                        id="printableSection" value="{{ translate('Proceed, If thermal printer is ready.') }}" />
                    <a href="{{ url()->previous() }}"
            ...
            <div class="initial-38-1">
                    ...
                    <tbody>
                        ...
                        @foreach ($order->details as $detail)
                            ...
                            @if ($detail->product_details != null)
                                @php($product = is_string($detail->product_details) ? json_decode($detail->product_details, true) : $detail->product_details)
                                <tr>
                                    <td class="">
                                        {{ $detail['quantity'] }}
                                    </td>
                                    <td class="">
                                        ...
                                        <span>{{ translate('Unit Price') }} :
                                            {{ Helpers::set_symbol($detail['price']) }}</span><br>
                                        <span>{{ translate('Qty') }} : {{ $detail['quantity'] }}</span><br>
                                        <span>{{ translate('Discount') }} :
                                            {{ Helpers::set_symbol($detail['discount_on_product']) }}</span>
                ...
                <div class="px-3">
                    <dl class="row text-right justify-content-center">
                        ...
                        <dt class="col-6">{{ translate('Tax / VAT') }}
                            {{ $vatStatus == 'included' ? translate('(included)') : '' }}:</dt>
                        ...
                        <dd class="col-6 font-20px">
                            {{ Helpers::set_symbol($subTotal + $del_c + $updatedTotalTax - $order['coupon_discount_amount'] - $order['extra_discount'] + $order['weight_charge_amount']) }}
                        </dd>
                    </dl>
                    <span
                        class="initial-38-5">---------------------------------------------------------------------------------</span>
