    ...
    @if (Helpers::module_permission_check(MANAGEMENT_SECTION['dashboard_management']))
        <div class="content container-fluid">
            <div class="page-header mb-0 pb-2 border-0">
                <h1 class="page-header-title text-107980">{{ translate('welcome') }}, {{ auth('admin')->user()->f_name }}
                </h1>
            ...
            <div class="card mb-10px">
                <div class="card-body">
                    ...
                    <div class="row g-2" id="business_metrics">
                        @include('admin-views.partials._dashboard-business-metrics', ['metrics' => $data])
                    </div>
                </div>
            </div>
                        ...
                        <div class="card h-100 bg-white">
                            <div class="card-body p-20px pb-0">
                                <div class="btn--container justify-content-between align-items-center">
                                    <h5 class="card-title mb-2">
                                        <img src="{{ asset('/public/assets/admin/img/order-statistics.png') }}"
                                            alt="" class="card-icon">
                                        ...
                                        <div class="d-flex flex-wrap statistics-btn-grp">
                                            <label>
                                                ...
                                                <span class="order-type"
                                                    data-order-type="yearOrder">{{ translate('This_Year') }}</span>
                                            </label>
                                            <label>
                                                <input type="radio" name="order__statistics" hidden>
                                                <span class="order-type"
                                                    data-order-type="MonthOrder">{{ translate('This_Month') }}</span>
                                            </label>
                                            <label>
                                                <input type="radio" name="order__statistics" hidden>
                                                <span class="order-type"
                                                    data-order-type="WeekOrder">{{ translate('This Week') }}</span>
                                            </label>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="card-body">
                                <div class="position-relative pie-chart">
                                    ...
                                    <div class="total--orders">
                                        <h3>{{ $data['pending_count'] + $data['ongoing_count'] + $data['delivered_count'] + $data['canceled_count'] + $data['returned_count'] + $data['failed_count'] }}
                                        </h3>
                                    ...
                                    <div class="mb-2">
                                        <div class="d-flex flex-wrap statistics-btn-grp">
                                            <label>
                                                ...
                                                <span class="earning-statistics"
                                                    data-earn-type="yearEarn">{{ translate('This_Year') }}</span>
                                            </label>
                                            <label>
                                                <input type="radio" name="earning__statistics" hidden>
                                                <span class="earning-statistics"
                                                    data-earn-type="MonthEarn">{{ translate('This_Month') }}</span>
                                            </label>
                                            <label>
                                                <input type="radio" name="earning__statistics" hidden>
                                                <span class="earning-statistics"
                                                    data-earn-type="WeekEarn">{{ translate('This Week') }}</span>
                                            </label>
                                        </div>
                                    </div>
                                </div>
    ...
    <script>
        ...
        var options = {
            series: [{
                ...
                data: [
                    {{ $orderStatisticsChart[1] }}, {{ $orderStatisticsChart[2] }},
                    {{ $orderStatisticsChart[3] }}, {{ $orderStatisticsChart[4] }},
                    {{ $orderStatisticsChart[5] }}, {{ $orderStatisticsChart[6] }},
                    {{ $orderStatisticsChart[7] }}, {{ $orderStatisticsChart[8] }},
                    {{ $orderStatisticsChart[9] }}, {{ $orderStatisticsChart[10] }},
                    {{ $orderStatisticsChart[11] }}, {{ $orderStatisticsChart[12] }}
                ],
            }],
            chart: {
                height: 316,
        ...
        var options = {
            series: [{{ $data['ongoing_count'] }}, {{ $data['delivered_count'] }}, {{ $data['pending_count'] }},
                {{ $data['canceled'] }}, {{ $data['returned'] }}, {{ $data['failed'] }}
            ],
            chart: {
                width: 320,
                type: 'donut',
            },
            labels: ['{{ translate('ongoing') }}', '{{ translate('delivered') }}', '{{ translate('pending') }}',
                '{{ translate('canceled') }}', '{{ translate('returned') }}', '{{ translate('failed') }}'
            ],
            dataLabels: {
                enabled: false,
                style: {
                    colors: ['#036BB7', '#544c30', '#6a5acd', '#ff00ff', '#0e0def', '#f51414']
                }
            },
            responsive: [{
                breakpoint: 1650,
            ...
            colors: ['#036BB7', '#544c30', '#6a5acd', '#0e0def', '#ff00ff', '#f51414'],
            fill: {
                colors: ['#036BB7', '#544c30', '#6a5acd', '#0e0def', '#ff00ff', '#f51414']
            },
            legend: {
        ...
        var optionsLine = {
            ...
            series: [{
                ...
                data: [{{ $earning[1] }}, {{ $earning[2] }}, {{ $earning[3] }}, {{ $earning[4] }},
